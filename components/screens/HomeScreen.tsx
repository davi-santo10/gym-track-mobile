...
import { Appbar, Card, Text, List, useTheme, Modal, Portal, AnimatedFAB, Button } from 'react-native-paper';
...
export function HomeScreen() {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['bottom', 'left', 'right']}>
        <Portal>
            <Modal visible={!!modalData} onDismiss={hideModal} contentContainerStyle={[styles.modalContainer, {backgroundColor: theme.colors.surface}]}>
                <View style={styles.modalContent}>
                    <Text variant="headlineSmall" style={styles.modalTitle}>Workout Complete!</Text>
                    <View style={styles.statRow}>
                        <Text variant="titleMedium">Time:</Text>
                        <Text variant="bodyLarge">{modalData ? formatDuration(modalData.duration) : ''}</Text>
                    </View>
                    <View style={styles.statRow}>
                        <Text variant="titleMedium">Total Weight Lifted:</Text>
                        <Text variant="bodyLarge">{modalData?.totalWeight.toLocaleString() ?? '0'} kg</Text>
                    </View>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 80 }}>
        <Card style={styles.card}>
            <View style={styles.planCardContainer}>
              {activeWorkout ? (
                <View>
                  <Card.Title title="Workout in Progress" titleVariant="titleLarge" />
                  <Card.Content> 
                    <Text variant="titleMedium" style={{marginBottom: 4}}>{activeWorkout.routine.name}</Text>
                    <Text variant="bodyMedium">A workout is currently active. Keep up the great work!</Text>
                  </Card.Content>
                </View>
              ) : (
                  ...
                  {todaysRoutine ? (
                    ...
                    <Card.Content>
                      <Text variant="bodyMedium">
                        You have no workouts scheduled for today. Resting is also essential!
                      </Text>
                    </Card.Content>
                  )}
                </View>
        ...
        {todaysRoutine ? (
          <Card 
            style={styles.card}
            onPress={lastLogForTodaysRoutine ? () => {
              router.push({ pathname: '/log/[id]', params: { id: lastLogForTodaysRoutine.id }});
            } : undefined}
          >
